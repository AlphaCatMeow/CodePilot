...
| [openai-compatible-thirdparty-provider.md](./openai-compatible-thirdparty-provider.md) | [handover/openai-compatible-thirdparty-provider.md](../handover/openai-compatible-thirdparty-provider.md) | OpenAI 兼容第三方服务商：多第三方服务配置、`/v1` 地址约束、模型发现与 Models 页启用策略、为什么不复用图片 preset |
