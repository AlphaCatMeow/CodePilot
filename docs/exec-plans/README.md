...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/chat-generated-image-assets.md](active/chat-generated-image-assets.md) | **Chat Generated Image Assets / 聊天生成图片资产化**：为 Joverna 等 chat/completions 返回的 Markdown base64 图片建立聊天缓存资产、大图预览、保存到本地、收藏到素材库、素材库路径持久化与后续缩略图/缓存清理方案；明确素材库 DB 不保存 base64 | ✅ Phase 1 已完成；Phase 2 缩略图/缓存清理待后续 |
...
| [active/global-agent-rules-loader.md](active/global-agent-rules-loader.md) | **Global Agent Rules Loader / 全局 Agent 规则加载**：将跨 Agent 共享规则抽到 `~/.agents/global-rules.md`，`AGENTS.md` / `CLAUDE.md` 只做 runtime 入口；后续由 CodePilot 会话启动器自动注入 global + runtime profile + native entry，并在 UI 中显示真实 loaded/skipped/truncated source breadcrumb | 📋 待 Claude Code 接手产品实现 |
...
| [active/openai-compatible-api-support.md](active/openai-compatible-api-support.md) | **OpenAI-Compatible API 支持**：支持第三方 OpenAI-compatible API；解除 DB 迁移删除阻断，新增 vendor preset，并限定 CodePilot Runtime 可用 | 📋 待开始 |
